 ...
 * @property BrainClient.EVENTS.EXPRESS_MODE    {string} Emitted when the client receives the response from the Brain indicating if express mode is enabled or not. The payload provided is a single boolean value, with `true` indicating that express mode IS enabled, and `false`, of course, indicating that express mode is disabled on the provisioned space.
